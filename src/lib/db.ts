...
export function parseBackup(value:unknown):Backup { if(!value||typeof value!=='object')throw new Error('This file is not a My Space backup.');const data=value as Partial<Backup>;if(data.format!=='my-space-backup'||data.version!==1||!Array.isArray(data.documents)||!Array.isArray(data.boards))throw new Error('Unsupported or damaged backup file.');return data as Backup }
